...
function rewrite(url, image = false) {
  ...
  return `${repo}/blob/${manifest.revision}/${local}`;
}
...
function page(title, body, gallery = false) {
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><meta name="theme-color" content="#9b2854"><title>${escape(title)}</title><link rel="icon" href="${base}icon.png"><link rel="stylesheet" href="${base}style.css?v=${manifest.revision}"></head><body><a class="skip-link" href="#content">Skip to content</a><header class="site-header"><a class="wordmark" href="${base}"><img src="${base}icon.png" width="32" height="32" alt="">ArchiveBox<span>Browser Extension</span></a><nav aria-label="Main navigation"><details class="project-menu"><summary>Projects</summary><div class="project-menu__links"><a href="https://archivebox.io/">ArchiveBox</a><a href="https://archivebox.github.io/ios-archivebox/">Apple apps</a><a href="https://archivebox.github.io/archivebox-browser-extension/" aria-current="page">Browser extension</a><a href="https://archivebox.github.io/abx-dl/">abx-dl</a><a href="https://archivebox.github.io/abx-plugins/">abx-plugins</a></div></details><a href="${base}screenshots/"${gallery ? ' aria-current="page"' : ''}>Screenshots</a><a href="${repo}">GitHub ↗</a></nav></header><div class="page-layout"><aside><details class="contents"><summary>On this page</summary><nav aria-label="README sections">${navigation}</nav></details></aside><main id="content" class="${gallery ? 'gallery' : 'markdown-body'}">${body}</main></div><footer><a href="${repo}/blob/${manifest.revision}/README.md">README source</a><a href="${base}screenshots/manifest.json">Screenshot manifest</a><a href="https://archivebox.io">ArchiveBox</a></footer><script src="${base}site.js?v=${manifest.revision}" defer></script></body></html>`;
}

...
- unlimitedStorage: Required in Chrome, Edge, and Firefox builds for larger screenshot, MHTML, and HTML captures in extension-local OPFS. Safari does not request this permission because Safari does not support it in this extension package.

Required permissions by browser:

- Chrome and Edge: storage, activeTab, contextMenus, unlimitedStorage.
- Firefox: storage, activeTab, contextMenus, unlimitedStorage.
- Safari: storage, activeTab, contextMenus.
...
- scripting: Requested only for local full-page screenshot scrolling. The extension first saves a visible-area screenshot without this permission, then requests scripting only if the user continues with the full-page scroll-and-stitch capture. When granted, the extension injects a runtime helper into the active tab to measure the page, scroll through screenshot tiles, and restore the original scroll position. If denied or unsupported, the already-saved visible-area screenshot is kept. The extension does not register an always-on content script.
